...
const checkSessionAvailability = async (subscriptionId, sessionDate = null) => {
  try {
    const subscription = await Subscription.findById(subscriptionId);
  ...
  try {
    ...
    if (coachId) {
      await User.findByIdAndUpdate(coachId, {
        $addToSet: { trainees: { trainee: userId } }
      });

      await User.findByIdAndUpdate(userId, {
        $addToSet: { coaches: { coach: coachId } }
      });
    }
    ...
    if (updateData.coachId && updateData.coachId !== subscription.coachId?.toString()) {
      ...
      if (subscription.coachId) {
        await User.findByIdAndUpdate(subscription.coachId, {
          $pull: { trainees: { trainee: subscription.userId } }
        });
      }
      ...
      if (newCoach && newCoach.role === 'coach') {
        await User.findByIdAndUpdate(updateData.coachId, {
          $addToSet: { trainees: { trainee: subscription.userId } }
        });

        await User.findByIdAndUpdate(subscription.userId, {
          $addToSet: { coaches: { coach: updateData.coachId } }
        });
      }
    }
...
const returnSession = async (req, res) => {
  try {
    const { id } = req.params;
